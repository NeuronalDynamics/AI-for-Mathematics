...
class PermutationModel(nn.Module):
    ...
    def plot_combined_heatmap(self, layer, indices):
        """
        Plots a combined heatmap of the weights for specified rows in a given layer.

        Parameters:
            layer (int): The layer for which the weights are to be visualized.
            indices (List[int]): A list of indices (i.e., rows) of the weights to be combined and visualized.
        """
        weights = self.layers[layer].weight
        combined_weights = weights[indices].sum(dim=0)
        maxVal = max(abs(combined_weights.min()), abs(combined_weights.max()))
        ...
        if layer == 0:
            ...
            for i in range(n):
                axes[i].imshow(
                    combined_weights[i * n : (i + 1) * n].unsqueeze(0).detach().numpy(),
                    cmap="bwr",
                    vmin=-maxVal,
                    vmax=maxVal,
                )
            ...
            fig.suptitle(f"Combined Rows {indices}")
            plt.tight_layout()
            plt.show()
        else:
            fig, ax = plt.subplots(1, 1, figsize=(combined_weights.shape[0], 1))
            ax.imshow(
                combined_weights.unsqueeze(0).detach().numpy(),
                cmap="bwr",
                vmin=-maxVal,
                vmax=maxVal,
            )
            ...
            ax.set_xticks(
                [x for x in range(combined_weights.shape[0])],
                [str(x + 1) for x in range(combined_weights.shape[0])],
            )
            fig.suptitle(f"Combined Rows {indices}")
            plt.tight_layout()
            plt.show()
